...
public class EmpleadoController {
    ...
    public ResponseEntity<Empleado> obtenerEmpleadoPorId(@PathVariable("id") long empleadoId){
        return this.empleadoService.getEmpleadoById(empleadoId).map(ResponseEntity::ok).orElseGet(
                () -> ResponseEntity.notFound().build());
    }

    @PostMapping("/{id}")
    public ResponseEntity<Empleado> actualizarEmpleado(@PathVariable("id") long empleadoId, @RequestBody Empleado empleado){
        return this.empleadoService.getEmpleadoById(empleadoId).map(empleadoGuardado -> {
            ...
            Empleado empleadoActualizado = this.empleadoService.updateEmpleado(empleadoGuardado);
